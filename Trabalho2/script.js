...
// Function to calculate soft shadow with penumbra effect
float calculatePenumbra(vec3 projectedTexcoord, float currentDepth) {
  float shadow = 0.0;
  vec2 texelSize = 1.0 / vec2(textureSize(u_projectedTexture, 0));
  
  // Sampling positions around the current texel
  for (int x = -1; x <= 1; x++) {
    for (int y = -1; y <= 1; y++) {
      float sampleDepth = texture(u_projectedTexture, projectedTexcoord.xy + vec2(x, y) * texelSize).r;
      shadow += sampleDepth < currentDepth ? 1.0 : 0.0;
    }
  }
  
  // Normalize the shadow value to be between 0 and 1
  return shadow / 9.0;
}
...
void main() {
  ...
  if (inRange) {
    float shadowFactor = calculatePenumbra(projectedTexcoord, currentDepth);
    shadowLight = mix(1.0, 0.5, shadowFactor);  
  }
